...
def select_league(option):
    mapping = {
        "KBO(1군)"  : "'KoreaBaseballOrganization'",
        "KBO(2군)"  : "'KBO Minors'",
        "AAA(마이너)": "'aaa'",
        "KBA(아마)" : "'TeamExclusive'",
    }
    return mapping.get(option, "'KoreaBaseballOrganization'")
...
def season_stand(player_df):
    sdf = player_df[player_df['game_year'] >= 2023]
    merged_base_df       = pivot_base_df(sdf, 'stand')
...
def season_stand_pitchname(player_df):
    sdf = player_df[player_df['game_year'] >= 2023]
    merged_base_df              = pivot_base_df(sdf, ['stand', 'pitch_name'])
    ...
    season_stand_pitchname_df   = season_stand_pitchname_df.reindex(
        ['4-Seam Fastball','2-Seam Fastball','Cutter','Slider','Sweeper','Curveball','Changeup','Split-Finger'],
        level='pitch_name'
    )
...
def season_pitchname(player_df):
    sdf = player_df[player_df['game_year'] >= 2023]
    merged_base_df      = pivot_base_df(sdf, 'pitch_name')
    ...
    season_pitchname_df = season_pitchname_df.reindex(
        ['4-Seam Fastball','2-Seam Fastball','Cutter','Slider','Sweeper','Curveball','Changeup','Split-Finger'],
        level='pitch_name'
    )
...
def stats_viewer(dataframe):
    df = dataframe[[
        'game_date','pitname','pa','ab','hit','walk','strikeout',
        'inplay_pit','exit_velocity','launch_angleX',
        'obp','slg','avg','ops',
        'weak','topped','under','flare','solid_contact','barrel'
    ]]
    return df.rename(columns={
        'game_date'    : '경기수',    'pitname'       : '투구수',
        'pa'           : '타석',      'ab'            : '타수',
        'hit'          : '피안타',    'walk'          : '볼넷',
        'strikeout'    : '삼진',      'inplay_pit'    : '인플레이/투구',
        'avg'          : '피안타율',  'obp'           : '피출루율',
        'slg'          : '피장타율',  'ops'           : '피OPS',
        'exit_velocity': '허용타구속도','launch_angleX': '허용발사각도',
        'weak'         : 'Weak',      'topped'        : 'Topped',
        'under'        : 'Under',     'flare'         : 'Flare',
        'solid_contact': 'Solid Contact','barrel'     : 'Barrel',
    })
...
def swing_viewer(dataframe):
    ...
    return df.rename(columns={
        'z%'       : '존투구%',    'z_swing%'  : '존스윙%',
        'z_con%'   : '존컨택%',    'z_inplay%' : '존인플레이%',
        'o%'       : '존외부%',    'o_swing%'  : '존외스윙%',
        'o_con%'   : '존외컨택%',  'o_inplay%' : '존외인플레이%',
        'f_swing%' : '초구스윙%',  'swing%'    : '스윙%',
        'whiff%'   : '헛스윙%',    'inplay_sw' : '스윙당인플레이%',
...
def stats_viewer_stand(dataframe):
    df = dataframe[[
        'stand','game_date','pitname','pa','ab','hit','walk','strikeout',
        'inplay_pit','exit_velocity','launch_angleX',
        'obp','slg','avg','ops',
        'weak','topped','under','flare','solid_contact','barrel'
    ]]
    return df.rename(columns={
        'stand'        : '타자유형',  'game_date'     : '경기수',
        'pitname'      : '투구수',    'pa'            : '타석',
        'ab'           : '타수',      'hit'           : '안타',
        'walk'         : '볼넷',      'strikeout'     : '삼진',
        'inplay_pit'   : '인플레이/투구','avg'         : '타율',
        'obp'          : '출루율',    'slg'           : '장타율',
        'ops'          : 'OPS',       'exit_velocity' : '타구속도',
        'launch_angleX': '발사각도',
        'weak'         : 'Weak',      'topped'        : 'Topped',
        'under'        : 'Under',     'flare'         : 'Flare',
        'solid_contact': 'Solid Contact','barrel'     : 'Barrel',
...
def swing_viewer_stand(dataframe):
    df = dataframe[[
        'stand','z%','z_swing%','z_con%','z_inplay%',
        'o%','o_swing%','o_con%','o_inplay%',
        'f_swing%','swing%','whiff%','inplay_sw',
        'plus_lsa4','approach'
    ]]
    return df.rename(columns={
        'stand'    : '타자유형',   'z%'        : '존투구%',
        'z_swing%' : '존스윙%',    'z_con%'    : '존컨택%',
        'z_inplay%': '존인플레이%','o%'        : '존외부%',
        'o_swing%' : '존외스윙%',  'o_con%'    : '존외컨택%',
        'o_inplay%': '존외인플레이%','f_swing%' : '초구스윙%',
        'swing%'   : '스윙%',      'whiff%'    : '헛스윙%',
        'inplay_sw': '스윙당인플레이%','plus_lsa4': 'LSA 4+',
        'approach' : '타격 어프로치',
    })
...
def stats_viewer_stand_pitchname(dataframe):
    df = dataframe[[
        'stand','pitch_name','game_date','pitname','pa','ab','hit','walk','strikeout',
        'inplay_pit','exit_velocity','launch_angleX',
        'obp','slg','avg','ops',
        'weak','topped','under','flare','solid_contact','barrel'
    ]]
    return df.rename(columns={
        'stand'        : '타자유형',  'pitch_name'    : '구종',
        'game_date'    : '경기수',    'pitname'       : '투구수',
        'pa'           : '타석',      'ab'            : '타수',
        'hit'          : '안타',      'walk'          : '볼넷',
        'strikeout'    : '삼진',      'inplay_pit'    : '인플레이/투구',
        'avg'          : '타율',      'obp'           : '출루율',
        'slg'          : '장타율',    'ops'           : 'OPS',
        'exit_velocity': '타구속도',  'launch_angleX' : '발사각도',
        'weak'         : 'Weak',      'topped'        : 'Topped',
        'under'        : 'Under',     'flare'         : 'Flare',
        'solid_contact': 'Solid Contact','barrel'     : 'Barrel',
    })
...
def swing_viewer_stand_pitchname(dataframe):
    df = dataframe[[
        'stand','pitch_name','z%','z_swing%','z_con%','z_inplay%',
        'o%','o_swing%','o_con%','o_inplay%',
        'f_swing%','swing%','whiff%','inplay_sw',
        'plus_lsa4','approach'
    ]]
...
def stats_viewer_pitchname(dataframe):
    df = dataframe[[
        'pitch_name','pitname','pa','ab','hit','walk','strikeout',
        'inplay_pit','exit_velocity','launch_angleX',
        'obp','slg','avg','ops',
        'weak','topped','under','flare','solid_contact','barrel'
    ]]
    return df.rename(columns={
        'pitch_name'   : '세부구종',  'pitname'       : '투구수',
        'pa'           : '타석',      'ab'            : '타수',
        'hit'          : '안타',      'walk'          : '볼넷',
        'strikeout'    : '삼진',      'inplay_pit'    : '인플레이/투구',
        'avg'          : '타율',      'obp'           : '출루율',
...
def swing_viewer_pitchname(dataframe):
    df = dataframe[[
        'pitch_name','z%','z_swing%','z_con%','z_inplay%',
        'o%','o_swing%','o_con%','o_inplay%',
        'f_swing%','swing%','whiff%','inplay_sw',
        'plus_lsa4','approach'
    ]]
    return df.rename(columns={
        'pitch_name': '세부구종',   'z%'        : '존투구%',
        'z_swing%'  : '존스윙%',    'z_con%'    : '존컨택%',
        'z_inplay%' : '존인플레이%','o%'        : '존외부%',
        'o_swing%'  : '존외스윙%',  'o_con%'    : '존외컨택%',
        'o_inplay%' : '존외인플레이%','f_swing%' : '초구스윙%',
...
def movement_dataframe(dataframe):
    ...
    grouped_df = dataframe.groupby(['game_year', 'pitch_name']).agg(agg_funcs)
    grouped_df['ver_break'] *= 100
    grouped_df['hor_break'] *= 100
    grouped_df = grouped_df.round({
        'rel_speed(km)': 1, 'release_spin_rate': 0,
        'ver_break': 1, 'hor_break': 1,
        'rel_height': 2, 'extension': 2, 'rel_side': 2,
    })
    ...
    grouped_df = grouped_df.rename(columns={
        'game_year'         : '연도',      'pitch_name'        : '구종',
        'pitname'           : '투구수',    'rel_speed(km)'     : '평균구속',
        'release_spin_rate' : '평균회전수','ver_break'         : '수직Mov',
        'hor_break'         : '수평Mov',   'rel_height'        : '릴리스 높이',
        'rel_side'          : '릴리스 좌우','extension'        : '익스텐션',
    })
